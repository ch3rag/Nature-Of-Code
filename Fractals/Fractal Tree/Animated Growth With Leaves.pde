...
ArrayList <PVector> leafPositions = new ArrayList <PVector> (); 
...
void branch(float x, float y, float length, float angle_) {
	...
	if(length < 5) {
		leafPositions.add(new PVector(x,y));
	    return;
	}

	branch(xl, yl, length, angle_ + angle + random(-0.3, 0.3));
	branch(xl, yl, length, angle_ - angle + random(-0.3, 0.3));
